...
class BlogController extends Controller
{
    ...
    public function rss_feed_page(Request $request)
    {
        $default_lang =  $request->lang ?? LanguageHelper::default_slug();
        return view(self::BASE_PATH.'blog.rss-import',compact('default_lang'));
    }
    
    
        public function rss_feed_store(Request $request)
        {

        $request->validate( [
            'rss_link_file' => 'required|string',
            'rss_item_import' => 'required|alpha_num',
        ]);
        
        $rss_link = $request->rss_link_file;
        $item = $request->rss_item_import;
        ...
        if($item > count($all_items) ){
            return redirect()->back()->with(FlashMsg::item_delete('You can not import avobe ' . count($all_items) . ' on this link'));
        }

        if($item < 1) {
            return redirect()->back()->with(FlashMsg::item_delete(' Please insert value avobe   0'));
        }
        ...
        foreach ($final_all_item ?? [] as $item) {
            ...
            if(!empty($item)){
                ...
                $blog->setTranslation('title',$request->lang,$title)
                    ->setTranslation('blog_content',$request->lang, $desc_condition)
                    ->save();
        ...
        return redirect()->back()->with(FlashMsg::item_new('Imported Successfully'));
